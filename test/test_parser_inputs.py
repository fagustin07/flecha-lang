...
class TestParser(unittest.TestCase):
    ...
    def test_el_analizador_lexico_genera_los_ast_esperados(self):
        input_dir = 'inputs'
        test_cases = [f for f in os.listdir(input_dir) if f.endswith('.input')]

        for test_case in test_cases:
            with self.subTest(test_case=test_case):
                base_name = test_case.split('.')[0]
                flecha_input_file = os.path.join(input_dir, f'{base_name}.input')
                expected_json_file = os.path.join(input_dir, f'{base_name}.expected')
